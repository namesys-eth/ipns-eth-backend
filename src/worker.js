...
async function handleCall(url, request, iterator) {
	...
	if (nature === 'read') {
		...
		let promise = new Promise((resolve, reject) => {
			connection.query(`SELECT ipns, ipfs, revision FROM events WHERE timestamp > ${Launch} AND user = ${user}`, function (error, results, fields) {
				if (error) {
					console.error('Error reading events from database:', error)
					return
				}
				const _ipns = results.map(row => row['ipns'])
				const _ipfs = results.map(row => row['ipfs'])
				const _timestamp = results.map(row => row['timestamp'])
				const _revision = results.map(row => row['revision'])
				const _sequence = results.map(row => row['sequence'])
				resolve(
					{
						type: 'history',
						data: {
	...
	if (nature === 'revision') {
		...
		promise = new Promise((resolve, reject) => {
			fs.writeFile(`${writePath}/version.json`,
				JSON.stringify(
					version
				), (err) => {
					if (err) {
